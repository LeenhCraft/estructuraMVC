...
class ProformaModel extends Mysql
{
    ...
    {
        $sql = "SELECT * FROM bib_pro_don WHERE pro_dni = '$dni'";
        $request = $this->select($sql);
        return $request;
    }
    
    public function listar($codReq)
    {

         $sql = "SELECT
                    a.idrequerimiento as id,
                    a.req_codigo AS cod,
                    c.art_nombre AS titulo,
                    b.det_cantidad AS cantidad
                    
                FROM
                    bib_requerimientos a
                INNER JOIN bib_detrequerimiento b ON
                    b.idrequerimiento = a.idrequerimiento
                INNER JOIN bib_articulos c ON
                    c.idarticulo = b.idarticulo
                    WHERE a.req_codigo = '$codReq'
                ";
